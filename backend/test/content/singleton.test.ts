...
describe("singleton content routes (about/contact-info)", () => {
  ...
  it("auto-creates every singleton on first GET, including intro (regression: empty-string defaults must not be `required`)", async () => {
    for (const path of ["/api/v1/intro", "/api/v1/about", "/api/v1/contact-info"]) {
